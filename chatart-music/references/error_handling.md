...
| `10061` | Content contains sensitive information | The prompt or lyrics contain sensitive/inappropriate words. Ask the user to modify the content and resubmit. |
...
Error occurred
├─ API error code?
│  ├─ 10058 → Inform user: insufficient credits, direct to https://app.chatartpro.com/subscriptions
│  ├─ 30027 → Inform user: insufficient permissions, direct to https://app.chatartpro.com/subscriptions
│  ├─ 4100 → Inform user: insufficient credits
│  ├─ 4007 → Wait and retry
│  ├─ 400x → Fix parameters, retry
│  ├─ 10061 → Content contains sensitive words. Ask user to modify prompt/lyrics and resubmit
│  ├─ 500x → Retry once, then suggest trying later
│  └─ 6001 → Security problem detected. Suggest revising the prompt
├─ Task status = fail?
