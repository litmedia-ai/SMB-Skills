...
Authenticate with ChatArt using the OAuth 2.0 Device Authorization Grant.
...
4. Saves credentials to `~/.ChatArt/credentials.json` on success
...
Credentials saved to ~/.ChatArt/credentials.json ✓
```
...
**Do NOT tell the user to register at chatartpro.com first.** The authorization page includes both login and registration. New users can sign up directly on that page.
...
Reads the pending session from `~/.ChatArt/pending_device.json` and resumes polling.
...
Logged in
  ...
  file:        /Users/you/.ChatArt/credentials.json
```
...
Use this command when you need to switch to a different ChatArt account on the same device.

### What it does
1. Calls the server to unbind the current device from the logged-in account
2. Removes the local credentials file (~/.ChatArt/credentials.json)
...
- Log out of your ChatArt web account on the website, OR
...
### When to use
- User wants to switch from ChatArt Account A to Account B

## Credential File

Saved at: `~/.ChatArt/credentials.json`. File permissions are set to `0600` (owner read/write only).

## Credential Load Priority

All modules use `shared/config.py` which loads credentials in this order:

1. Environment variables `CHATART_UID` + `CHATART_API_KEY` (CI/scripting)
2. `~/.ChatArt/credentials.json` (set by `auth.py login`)
3. Error — prompts to run `auth.py login`

## Agent Rules

- **NEVER** ask the user to run `python auth.py login` or any other command themselves.
- **NEVER** ask the user to set `CHATART_UID` or `CHATART_API_KEY` env vars manually.
