"""Load ChatArt API credentials.

Priority order:
1. Environment variables CHATARTPRO_UID + CHATARTPRO_API_KEY  (CI / backwards compat)
2. Credential file ~/.ChatArt/credentials.json           (set by auth.py login)
...
def _load_from_file() -> dict | None:
	...
	try:
		...
		uid = data.get("identity_id", "").strip()
		if api_key and uid:
			return {"api_key": api_key, "uid": uid}
...
def load_config() -> dict:
	...
	uid = os.environ.get("CHATARTPRO_UID", "").strip()
	api_key = os.environ.get("CHATARTPRO_API_KEY", "").strip()
	...
	missing_env = []
	if not uid:
		missing_env.append("CHATARTPRO_UID")
	if not api_key:
		missing_env.append("CHATARTPRO_API_KEY")

	print(
		"Error: ChatArt credentials not found.\n\n"
		...
		'  export CHATARTPRO_UID="<your-litmedia-uid>"\n'
		'  export CHATARTPRO_API_KEY="<your-api-key>"\n',
		file=sys.stderr,
	)
	sys.exit(1)
